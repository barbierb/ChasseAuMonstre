...
public class Engine {
		...
		while(getWinner() == null) {
			...
			if(this.tourMonstre) {
				...
				if(this.monstre instanceof MonstreIA) {
					System.out.println("--- tour monstre IA ---");
					//TODO BOSSER LES IA ICI
				} else {
					System.out.println("--- tour monstre JOUEUR ---");
				...
				if(this.chasseur instanceof ChasseurIA) {
					System.out.println("--- tour chasseur IA ---");
					//TODO BOSSER LES IA ET LA
				} else {
					System.out.println("--- tour chasseur JOUEUR ---");
	...
	private Position saisiePosition() {
		...
		do {
			...
			System.out.println("     A->"+(char)(getPlateau().getCases().length+'A')+" 1->"+getPlateau().getCases()[0].length);
			choix = Clavier.lireString();
		} while(choix.length()!=2 
				|| (choix.charAt(0)<'A')
				|| (choix.charAt(0)>(getPlateau().getCases().length+'A')) 
				|| (choix.charAt(1)<'1') 
				|| (choix.charAt(1)>=(getPlateau().getCases()[0].length+'1')));
